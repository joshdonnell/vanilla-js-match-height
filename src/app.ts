...
class MatchHeight implements IMatchHeight {
  ...
  findElements() {
    ...
    if (
      this.options.parent &&
      document.querySelectorAll(this.options.parent).length
    ) {
      // Loop over to get the elements
      document
        .querySelectorAll(this.options.parent)
        .forEach((parentGroup: HTMLElement) => {
    ...
    utilSetTimeout(() => {
      if (this.elements) {
        ...
        this.elements.forEach((group, index) => {
          ...
          if (group.parent) {
            console.log(
              `Element: ${this.element} Parent: ${group.parent.tagName}.${group.parent.className}`
            )
          } else {
        ...
        console.log(
          `⛔ No cached MatchHeight elements found for "${this.element}" \n\n Make sure this element exists on the current page.`
        )
      }
...
const utilSetTimeout = <T extends () => void>(callback: T, delay: number) => {
  let startTime = performance.now()

  const handleCallback = (currentTime: number) => {
    const elapsedTime = currentTime - startTime

    if (elapsedTime >= delay) {
      callback()
    } else {
      requestAnimationFrame(handleCallback)
    }
  }
